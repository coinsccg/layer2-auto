...
def run():
    ...
    st.metamask_change_network(metamask_password)
    st.change_network("以太坊 Ethereum 主网络")
    st.login_argent(argent_password)
    st.argent_change_network("Ethereum Mainnet")
    # 转账
    st.deposit(int(start_account), int(end_account), token)

    print("全部账号完成存款， 请等待5分钟后进行取款！")
